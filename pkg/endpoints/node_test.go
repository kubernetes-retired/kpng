...
func ExampleForNodeWithTopology() {
	...
	store.Update(func(tx *proxystore.Tx) {
		tx.SetService(&localnetv1.Service{
			...
			Name:      "test",
			Type:      "ClusterIP",
			IPs:       &localnetv1.ServiceIPs{ClusterIP: "10.1.2.3"},
		...
		tx.SetEndpointsOfSource("test", "test-abcde", []*localnetv1.EndpointInfo{
			{
				...
				ServiceName: "test",
				Endpoint:    &localnetv1.Endpoint{IPs: localnetv1.NewIPSet([]string{"10.2.0.1"})},
			...
			},
			{
				...
				SourceName:  "test-abcde",
				ServiceName: "test",
				Endpoint:    &localnetv1.Endpoint{IPs: localnetv1.NewIPSet([]string{"10.2.1.1"})},
