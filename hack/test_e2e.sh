...
function install_kpng {
    ...
    E2E_BACKEND_ARGS="'local', '--api=${KPNG_SERVER_ADDRESS}', 'to-${E2E_BACKEND}', '--v=${KPNG_DEBUG_LEVEL}', '--exportMetrics','9090'"
    if [[ "${E2E_DEPLOYMENT_MODEL}" == "single-process-per-node" ]]; then
        E2E_BACKEND_ARGS="'kube', '--kubeconfig=/var/lib/kpng/kubeconfig.conf', 'to-local', 'to-${E2E_BACKEND}', '--v=${KPNG_DEBUG_LEVEL}', '--exportMetrics','9090'"
    fi
