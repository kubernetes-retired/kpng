...
func (t *iptables) sync() {
	...
	// part of the proxy...
	preexistingFilterChains := make(map[util.Chain][]byte)
	t.existingFilterChainsData.Reset()
	err := t.iptInterface.SaveInto(util.TableFilter, t.existingFilterChainsData)
	...
	err = t.iptInterface.SaveInto(util.TableNAT, t.iptablesData)
	if err != nil { // if we failed to get any rules
		klog.ErrorS(err, "Failed to execute iptables-save, syncing all rules")
	} else { // otherwise parse the output
