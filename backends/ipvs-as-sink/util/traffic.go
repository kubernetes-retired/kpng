...
func NewDetectLocalByCIDR(cidr string, ipt IPTableInterface) (LocalTrafficDetector, error) {
...
func (d *detectLocalByCIDR) JumpIfLocal(args []string, toChain string) []string {
	...
	klog.V(4).Infof("Detect Local By CIDR", "cidr", d.cidr, "jumpLocal", line)
	return line
}

func (d *detectLocalByCIDR) JumpIfNotLocal(args []string, toChain string) []string {
	line := append(args, "!", "-s", d.cidr, "-j", toChain)
	klog.V(4).Infof("Detect Local By CIDR", "cidr", d.cidr, "jumpNotLocal", line)
	return line
}
